...
//Prácitca 6 del aparcamiento (Funciones II)
...
void main(void) {
	...
	char opcion_menu1_1,opcion;
	int estado, precio, orden, opcion_menu2;
	int	num_plazas_coche_libre, num_plazas_moto_libre, num_plaza;
	...
	while ((opcion_menu1 = menu()) != 'S') {
		...
		case 'R'://Reservar plaza
			...
			if (estado == 0) {
				...
				num_plaza=buscarPlazaLibre(aparcamiento, NPLAZAS, tipoauto);
				...
				if (num_plaza >= 0) {
					...
					strcpy(aparcamiento[num_plaza].matricula, matricula_aux);
					aparcamiento[num_plaza].hora_entrada = (int)ltime;
					aparcamiento[num_plaza].estado = 1;
				}
				else {
					printf("\n\nNo quedan plazas disponibles para");
					if ((tipoauto == 'c') || (tipoauto == 'C'))
						printf(" coche\n\n");
					else if ((tipoauto == 'm') || (tipoauto == 'M'))
						printf(" moto\n\n");
					system("Pause()");
				}
			}
		...
		case 'E'://Estado de las plazas
			...
			printf("\n\n");
			num_plazas_coche_libre = 0;
			num_plazas_moto_libre = 0;
			recuento(aparcamiento, NPLAZAS, &num_plazas_coche_libre,&num_plazas_moto_libre);
			printf("Recuento de plazas libres: \n\n>COCHE:  %d\n>MOTO:   %d", 
				num_plazas_coche_libre, num_plazas_moto_libre);
			printf("\n\n");
			system("Pause()");
			break;
		...
		case 'B': //Buscar vehiculo por matricula
			...
			num_plaza = buscarVehiculo(aparcamiento,NPLAZAS,matricula_aux);
			
			if (num_plaza > -1) {
				printf("\n%s >> Su busqueda se encuentra en la Plaza %d", matricula_aux, num_plaza + 1);
				if (aparcamiento[num_plaza].tipo == 'C')
					printf(" - Coche\n\n");
				else
					printf(" - Moto\n\n");
			}
...
void inicializar(plaza aparcamiento[], int dim) {
	int i;
...
int buscarPlazaLibre(plaza aparcamiento[], int dim, char tipo) {
	...
	case'M':
		...
		for (j = (((int)NPLAZAS*0.75) + 1); j < dim; j++) {
			if (aparcamiento[j].estado == 0) {
				return j;
			}
		}
